...
export async function upsertChatRequests(
  ...
  const uniqueRequests = dedupeChatRequests(
    requests.map((request) => normalizeTokenTotals(request)),
  );
...
function normalizeTokenTotals(request: CopilotChatRequest): CopilotChatRequest {
  if (request.inputTokens === null && request.outputTokens === null) {
    return { ...request, totalTokens: null, tokenSource: "missing-in-copilot-otel" };
  }

  if (request.inputTokens === null || request.outputTokens === null) {
    return { ...request, totalTokens: null, tokenSource: "partial-in-copilot-otel" };
  }
