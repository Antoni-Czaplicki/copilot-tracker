...
export const sessions = pgTable('sessions', {
  ...
  githubId: bigint('github_id', { mode: 'number' }).notNull().references(() => users.githubId, { onDelete: 'cascade' }),
  createdAt: text('created_at').notNull(),
...
export const trackerEvents = pgTable('tracker_events', {
  ...
  githubId: bigint('github_id', { mode: 'number' }).references(() => users.githubId, { onDelete: 'set null' }),
}, (table) => [
...
export const chatRequests = pgTable('chat_requests', {
  ...
  promptTokenDetails: jsonb('prompt_token_details').$type<Array<{
...
export const githubCopilotBillingUsage = pgTable('github_copilot_billing_usage', {
  id: text('id').primaryKey(),
  scopeType: text('scope_type', { enum: ['user', 'organization', 'enterprise'] }).notNull(),
  ...
  index('github_copilot_billing_usage_scope_idx').on(table.scopeType, table.scope),
