...
export function summarizeRequests(requests: CopilotChatRequest[]): SummaryMetrics {
  const requestCount = requests.length;
  const missingTokenCount = requests.filter((request) => request.totalTokens === null).length;
  const inputTokens = requests.reduce((total, request) => total + (request.inputTokens ?? 0), 0);
  const outputTokens = requests.reduce((total, request) => total + (request.outputTokens ?? 0), 0);
  const totalTokens = requests.reduce((total, request) => total + (request.totalTokens ?? 0), 0);
  return {
    requestCount,
    missingTokenCount,
    inputTokens,
...
export function publicLeaderboard(database: TrackerDatabase): LeaderboardRow[] {
  const grouped = new Map<string, Array<CopilotChatRequest & { githubLogin: string | null; githubId: number | null }>>();
...
export function taskSummaries(requests: CopilotChatRequest[]): TaskSummaryRow[] {
...
export function developerTaskSummaries(requests: Array<CopilotChatRequest & { githubLogin?: string | null; githubId?: number | null }>): DeveloperTaskSummaryRow[] {
  const grouped = new Map<string, Array<CopilotChatRequest & { githubLogin?: string | null; githubId?: number | null }>>();
  for (const request of requests) {
    const key = [
...
export function modelSummaries(requests: CopilotChatRequest[]): ModelSummaryRow[] {
  const grouped = new Map<string, CopilotChatRequest[]>();
  for (const request of requests) {
    const model = request.modelId ?? request.resolvedModel ?? request.modelName ?? 'unknown';
