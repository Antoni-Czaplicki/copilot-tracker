...
export async function POST(request: NextRequest) {
  ...
  const validRequests = body.requests.filter((chatRequest) => (
    chatRequest.requestRecordId && chatRequest.sessionId && chatRequest.workspaceId
  ));
  await upsertChatRequests(validRequests, user);

  return NextResponse.json({ ok: true, upserted: validRequests.length }, { status: 202 });
}
