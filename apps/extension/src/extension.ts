...
export function activate(context: vscode.ExtensionContext) {
	...
	statusItem = vscode.window.createStatusBarItem(`${extensionId}.status`, vscode.StatusBarAlignment.Left, 98);
	statusItem.name = 'Copilot Tracker';
	statusItem.command = `${extensionId}.setTask`;
	context.subscriptions.push(statusItem);

	context.subscriptions.push(vscode.commands.registerCommand(`${extensionId}.setTask`, () => setTask(context, client)));
	context.subscriptions.push(vscode.commands.registerCommand(`${extensionId}.useBranchTask`, () => useBranchTask(context, client)));
	context.subscriptions.push(vscode.commands.registerCommand(`${extensionId}.syncCopilotSessions`, () => syncCopilotSessions(context, client)));
	context.subscriptions.push(vscode.commands.registerCommand(`${extensionId}.openDashboard`, () => openDashboard()));
	context.subscriptions.push(vscode.commands.registerCommand(`${extensionId}.showContext`, () => showContext(context)));

	void refreshContext(context, client, 'extension-started');
	void syncCopilotSessions(context, client);

	const storageRoot = getTrackerConfig().chatStoragePath || getDefaultWorkspaceStorageRoot();
	const watchers = createChatSessionWatcher(storageRoot, () => scheduleSync(context, client, 350));
	context.subscriptions.push(...watchers);

	const branchPoller = setInterval(() => void refreshContext(context, client), 10_000);
...
async function setTask(context: vscode.ExtensionContext, client: TrackerClient) {
	...
	const task = await vscode.window.showInputBox({
		...
		ignoreFocusOut: true,
		validateInput: (value) => value.trim().length === 0 ? 'Task cannot be empty.' : undefined,
	});
	...
	await refreshContext(context, client, 'task-changed', { source: 'manual-input' });
	await syncCopilotSessions(context, client);
}

async function useBranchTask(context: vscode.ExtensionContext, client: TrackerClient) {
	await setSelectedTask(context, undefined);
	await refreshContext(context, client, 'task-changed', { source: 'branch-default' });
	await syncCopilotSessions(context, client);
}
...
async function maybePromptForBranchTask(
	...
	if (choice === 'Switch task') {
		await setSelectedTask(context, undefined);
		await refreshContext(context, client, 'task-changed', { source: 'branch-change-prompt' });
		await syncCopilotSessions(context, client);
	}
}
...
function updateStatusItem(snapshot: WorkspaceContext) {
	...
	const branch = snapshot.branch ? `$(git-branch) ${snapshot.branch}` : 'no git branch';
...
function scheduleSync(context: vscode.ExtensionContext, client: TrackerClient, delayMs: number) {
	clearPendingSync();
...
async function pollForChatStorageChanges(context: vscode.ExtensionContext, client: TrackerClient) {
	const config = getTrackerConfig();
	...
	try {
		const signature = await getChatSessionSignature(config.chatStoragePath || getDefaultWorkspaceStorageRoot());
...
async function syncCopilotSessions(context: vscode.ExtensionContext, client: TrackerClient) {
	const config = getTrackerConfig();
	if (!config.readVsCodeChatStorage) {
		return;
	}
	...
	try {
		const workspaceContext = currentWorkspaceContext ?? await buildWorkspaceContext(context);
		const requests = await readCopilotChatRequests(workspaceContext, config);
		await client.sendChatRequests(requests);
		lastSyncStats = {
			requestCount: requests.length,
			tokenCount: requests.reduce((total, request) => total + (request.totalTokens ?? 0), 0),
			missingTokenCount: requests.filter((request) => request.totalTokens === null).length,
		};
