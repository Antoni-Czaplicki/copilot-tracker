...
13. [PARTIAL] WorkItemPicker search threshold, error-message mapping, empty states, and keyboard active-index transitions now have direct coverage; rendered keyboard/listbox behavior still needs component/browser tests.
...
7. [FIXED in `23e4df9` and live verified] `/api/health` now normalizes blank/`unknown` build metadata, supports common source env fallbacks, and uses a generated Docker build-info module from minimal Git refs. Strict production smoke now proves the exact deployed commit.
...
28. [FIXED in `311bd56` and live verified] Real VS Code extension sign-in requested Azure DevOps scopes through VS Code's Microsoft auth provider and failed with Microsoft `AADSTS65002`. The extension now uses tracker web sign-in plus a VS Code URI callback, stores only a tracker session token in SecretStorage, and leaves Azure DevOps tokens server-side.
29. [FIXED in `ae3d4e4`] Real VS Code OTel syncing repeatedly rewrote Copilot's `outfile` setting, rebuilt lifecycle timers, and emitted redundant event POSTs. The extension now coalesces lifecycle rebuilds, uses the active OTel path during poll/sync, and ignores configuration-change events caused by its own exporter setup. The rebuilt VSIX stayed quiet after reload, and both GitHub Actions workflows passed for the pushed commit.
