...
0. [P1] Production Azure login now passes the previous `invalid_client` token-exchange blocker after the callback was moved to `Web`, but full signed-in production E2E is still blocked by `auth_code=profile_or_org_check_failed`. Deployed diagnostics prove Azure profile lookup succeeds (`profileResult=ok`, HTTP 200) and the configured organization membership check does not match (`orgMembershipResult=not_matched`, HTTP 200, account count 1). `9a3acb1` normalized old-style org URLs but production still failed. A new local fix probes the configured Azure DevOps org WIQL endpoint directly after account-name matching fails; after deploy, rerun Chrome login. If the probe also fails, fix `AZURE_DEVOPS_ORG` or the signed-in user's Azure DevOps organization membership/visibility, then rerun dashboard/work-item E2E.
