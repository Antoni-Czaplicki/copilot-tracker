...
29. [FIXED pending deploy] Real VS Code OTel syncing repeatedly rewrote Copilot's `outfile` setting, rebuilt lifecycle timers, and emitted redundant event POSTs. The extension now coalesces lifecycle rebuilds, uses the active OTel path during poll/sync, and ignores configuration-change events caused by its own exporter setup.
