...
| DEP-367 | Deployment/Dokploy | PASS | Dokploy deploys generated TypeScript build metadata module for the latest commit | Dokploy UI/logs | Deployment for `23e4df9` completed and logs showed generated module creation before `next build` |
| DEP-368 | Deployment/Smoke | PASS | Strict production smoke proves exact deployed commit without known-stale mode | Live prod smoke | `pnpm smoke:production -- --expect-sha 23e4df9` passed; `/api/health` reports the exact full SHA and non-unknown build time |
| DEP-369 | Deployment/CI | PASS | Log-only exact proof commit passes GitHub Actions | GitHub Actions | `f90a5b0` CI and Build extension completed successfully |
| DEP-370 | Deployment/Smoke | FAIL | Production should immediately serve the log-only commit SHA | Live prod smoke | Strict smoke for `f90a5b0` failed only the SHA check because production still served prior verified `23e4df9`; all other gates passed |
| WEB-371 | Web/Dashboard | PASS | Signed-in dashboard work-item search endpoint handles empty real Azure DevOps results without auth failure | Chrome/CDP + production dashboard | Safe live queries returned HTTP 200 with zero matches; no provider/auth failure surfaced |
| API-372 | API/Azure DevOps | PASS | Text work-item search tries substring fallback after empty words-query results | Automated | Added web lib regression test with empty first WIQL response and matching second response |
| WEB-373 | Web/UX | PASS | Numeric task IDs use a clearer Azure DevOps no-match status | Automated | Added WorkItemPicker helper test for numeric and text empty-state messages |
| CHECK-374 | Quality | PASS | Broad local validation passes after work-item search improvement | CLI | Web tests/typecheck/lint, production-style web build, smoke tests, repo typecheck/lint, extension tests, root `pnpm test`, and diff check passed |
| DEP-375 | Deployment/Smoke | PASS | Production serves the work-item search commit after Dokploy build and Reload | Live prod smoke | Strict smoke passed for `6477f9c` after app Reload switched production from previous `23e4df9` |
...
| AUTH-383 | Auth/API | PASS | Azure OAuth callback success creates a tracker session and clears PKCE cookies | Automated | Added route-level handler test with injected auth/session dependencies; web tests passed |
| AUTH-384 | Auth/API | PASS | Azure OAuth callback session creation failures redirect safely without exposing exception details to the client | Automated | Added route-level handler test for `callback_failed`, `auth_ref`, cookie clearing, and server-side diagnostic log; web tests passed |
| CHECK-385 | Quality | PASS | Broad local validation passes after Azure OAuth callback route coverage | CLI | Web tests/typecheck/lint, repo typecheck/lint, production-style web build, extension compile/test, smoke tests, root tests, diff check, and strict production smoke for deployed `d19e76c` passed |
| DEP-386 | Deployment/Smoke | PASS | Production serves Azure OAuth callback route coverage commit | Live prod smoke | Strict smoke passed for `6ed152d` after Dokploy finished rolling the build |
| EXT-387 | Extension/Task History | PASS | Clearing a manual task on a no-task branch records an explicit no-task history entry | Automated | Added `shouldRecordTaskHistoryEntry` coverage; extension tests passed |
| EXT-388 | Extension/Task History | PASS | Historical no-task entries do not fall back to the current selected task during OTel attribution | Automated | Updated resolver coverage so explicit clears resolve to `selectedTask: null`; extension tests passed |
| EXT-389 | Extension/Multi-workspace | PASS | Latest task-history lookup is scoped to the active workspace | Automated | Added `latestTaskHistoryForWorkspace` coverage; extension tests passed |
| CHECK-390 | CI | PASS | Extension task clear attribution fix passes CI and VSIX package workflow | GitHub Actions | `9d298f5` CI and Build extension completed successfully |
| DEP-391 | Deployment/Smoke | PASS | Production remains healthy after extension-only task-history commit | Live prod smoke | Strict smoke passed for deployed app commit `6ed152d`; no Dokploy Reload forced for extension-only change |
| EXT-392 | Extension/Multi-workspace | PASS | Branch-change prompt suppression is scoped by workspace id | Automated | Added `branchTaskSwitchPrompt` coverage for repeated prompts in one workspace and same transition in another workspace |
| EXT-393 | Extension/Task UX | PASS | Branch task prompt appears only when a new default task differs from the current manual selection | Automated | Added prompt helper coverage for no-default and already-on-default skip cases |
| CHECK-394 | CI | PASS | Workspace-scoped branch prompt hardening passes CI and VSIX package workflow | GitHub Actions | `25da717` CI and Build extension completed successfully |
| EXT-395 | Extension/Repository Matching | PASS | GitHub SSH and HTTPS remotes normalize to the same workspace identity | Automated | Added `normalizeRepositoryRemoteUrl` coverage; extension tests passed |
| EXT-396 | Extension/Repository Matching | PASS | Azure DevOps SSH and HTTPS remotes normalize to the same workspace identity | Automated | Added Azure DevOps SSH `/v3/org/project/repo` and HTTPS `_git` coverage; extension tests passed |
| CHECK-397 | CI | PASS | Repository remote normalization extension hardening passes CI and VSIX package workflow | GitHub Actions | `0416c27` CI and Build extension completed successfully |
| DEP-398 | Deployment/Smoke | PASS | Production web remains healthy after extension-only commits | Live prod smoke | Strict smoke passed for deployed web commit `6ed152d`; later extension-only commits did not require Dokploy Reload |
| EXT-399 | Extension/OTel | PASS | Full OTel ingestion accepts Azure DevOps SSH telemetry for an HTTPS workspace remote | Automated | Added `readCopilotOtelRequests` regression with Azure DevOps SSH request metadata and HTTPS workspace context |
| EXT-400 | Extension/OTel | PASS | Full OTel ingestion filters unrelated repositories from the same OTel file | Automated | Same regression includes a second Azure DevOps repository and asserts only the matching request is returned |
| CHECK-401 | Quality | PASS | Broad validation passes after OTel remote filtering regression | CLI | Extension compile/test, repo typecheck/lint, root tests, and strict production smoke for `6ed152d` passed |
| CHECK-402 | CI | PASS | OTel remote filtering regression passes CI and VSIX package workflow | GitHub Actions | `2283a85` CI and Build extension completed successfully |
| EXT-403 | Extension/OTel | PASS | Full OTel ingestion resolves historical branch/default/manual/clear task states by request timestamp | Automated | Added multi-request `readCopilotOtelRequests` regression using `createTaskResolverFromHistory`; extension suite is 35 tests |
| CHECK-404 | Quality | PASS | Broad validation passes after historical task attribution regression | CLI | Extension compile/test, repo typecheck/lint, root tests, and strict production smoke for `6ed152d` passed |
| EXT-405 | Extension/VS Code | PASS | Real no-task clear does not leave stale task attribution on a later Copilot request | Real VS Code + production dashboard | After clearing the task on a no-task branch, dashboard showed the later request under `No task` with the expected token split/cost |
| EXT-406 | Extension/VS Code | PASS | Real branch prompt accept/dismiss behavior preserves the intended selected task | Real VS Code logs/status + production dashboard | Accepted prompt switched to the detected task; dismissed prompt preserved the manual task and status item |
| EXT-407 | Extension/Multi-workspace | PASS | Same branch transition in another workspace is not globally suppressed | Real VS Code logs/status | Workspace B showed the prompt for the same branch transition previously seen in workspace A, then preserved the manual task when dismissed |
| EXT-408 | Extension/OTel | PASS | Repo-less Copilot OTel log records are accepted only when their chat `session.id` matches this workspace | Automated | Added `readCopilotOtelRequests` repo-less exact-session coverage; extension suite is 40 tests |
| EXT-409 | Extension/OTel | PASS | Timestamp-only session title matches from another workspace cannot admit repo-less OTel records | Automated | Added regression for a nearby different chat session id returning zero requests |
| EXT-410 | Extension/OTel | PASS | Patched parser keeps the real Azure DevOps SSH workspace to its own global OTel chat request | Local replay | Real global OTel + real VS Code workspace storage replay returned five records for workspace A and exactly one Azure-SSH record for workspace B |
| CHECK-411 | Quality | PASS | Broad validation passes after repo-less OTel attribution fix | CLI | Extension compile/test, repo typecheck/lint, root `pnpm test`, VSIX package/install, and real OTel replay passed |
