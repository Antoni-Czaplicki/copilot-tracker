...
Docker builds also generate `apps/web/src/generated/buildInfo.generated.ts`
from explicit metadata, common source metadata variables, or minimal `.git`
refs before `next build`, so the server bundle can report the deployed commit.
Runtime environment variables still win when set, and the final image does not
keep the full `.git` directory.
